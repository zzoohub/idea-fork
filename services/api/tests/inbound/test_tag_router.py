...
@pytest.mark.asyncio
async def test_list_product_tags_returns_data():
    tag_repo = AsyncMock()
    tag_repo.list_tags = AsyncMock(return_value=[])
    tag_repo.list_product_tags = AsyncMock(
        return_value=[make_tag(1, "saas", "SaaS"), make_tag(2, "mobile", "Mobile")]
    )

    app = build_test_app(tag_repo=tag_repo)
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get("/v1/tags/by-products")
...
@pytest.mark.asyncio
async def test_list_product_tags_with_params():
    tag_repo = AsyncMock()
    tag_repo.list_tags = AsyncMock(return_value=[])
    tag_repo.list_product_tags = AsyncMock(return_value=[make_tag(1, "saas", "SaaS")])

    app = build_test_app(tag_repo=tag_repo)
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get("/v1/tags/by-products?days=30&limit=10")

    assert resp.status_code == 200
    tag_repo.list_product_tags.assert_called_once_with(30, 10)


@pytest.mark.asyncio
async def test_list_product_tags_has_cache_header():
    tag_repo = AsyncMock()
    tag_repo.list_tags = AsyncMock(return_value=[])
    tag_repo.list_product_tags = AsyncMock(return_value=[])
