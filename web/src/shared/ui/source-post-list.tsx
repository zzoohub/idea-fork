...
import { formatRelativeTime, formatNumber, sanitizeExternalUrl } from "@/shared/lib/utils";
...
export function SourcePostList({ posts, compact = false }: SourcePostListProps) {
  return (
    <div className={compact ? "space-y-3" : "space-y-2"}>
      {posts.map((post) => (
        <a
          ...
          className={`flex items-start gap-3 rounded-md hover:bg-accent transition-colors group min-h-11 ${
            compact ? "p-2 -mx-2" : "p-3 -mx-3"
          }`}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className={`text-sm leading-relaxed ${compact ? "line-clamp-1" : "line-clamp-2"}`}>
              &ldquo;{post.excerpt}&rdquo;
            </p>
            <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
              {compact && <span>{post.platformSubSource}</span>}
